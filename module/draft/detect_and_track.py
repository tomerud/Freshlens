...
def save_tracker_state(tracker, file_path="tracker_state.json"):
    """
    Save the current tracking IDs and bounding boxes.
    """
    tracked_objects = []
    ...
    for track in tracker.tracker.tracks:
        if not track.is_confirmed() or track.time_since_update > 1:
            continue
        bbox = track.to_tlbr()  # Convert bbox to [min_x, min_y, max_x, max_y]
        tracked_objects.append({
            "track_id": track.track_id,
            "bbox": bbox.tolist()  # Convert NumPy array to a list
        })

    with open(file_path, "w") as f:
        json.dump(tracked_objects, f)


def load_tracker_state(file_path="tracker_state.json"):
    """
    Load the previously saved tracking IDs and bounding boxes.
    """
    ...
    with open(file_path, "r") as f:
        try:
            return json.load(f)  # Load track states
...
def update_tracker(
    tracker, detections, frame, last_frame_objects, detected_products, debug, class_list, saved_tracks=None
):
    """
    Update the DeepSORT tracker while keeping the same track ID across multiple videos.
    Only applies saved tracks in the first frame.
    """
    tracks = tracker.update_tracks(detections, frame=frame)
    ...
    for track in tracks:
        ...
        class_id = track.get_det_class() if track.get_det_class() is not None else -1
        name = class_list[class_id]

        # **Assign previous tracking ID only in the first frame**
        if saved_tracks is not None:
            for saved in saved_tracks:
                if iou(bbox, saved["bbox"]) > 0.7:  # If IoU is high, assume same object
                    track.track_id = saved["track_id"]
                    break

        if debug:
            print(f"DeepSORT Track - ID: {track.track_id}, BBox: {bbox}, Class: {name}")
...
def process_video(
    ...
    last_frame = None
    saved_tracks = load_tracker_state()
    first_frame = True
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
        ...
        update_tracker(tracker, detections, frame, last_frame_objects, detected_products, debug, class_list,saved_tracks if first_frame else None)
        first_frame = False
